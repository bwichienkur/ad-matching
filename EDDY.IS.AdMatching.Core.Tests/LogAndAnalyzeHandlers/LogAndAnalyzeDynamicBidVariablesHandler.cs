...
public class LogAndAnalyzeDynamicBidVariablesHandler : IChainHandler<AdMatchingModel>, IStopWatchable
{
    public IChainHandler<AdMatchingModel> DecoratedClass { get; set; }
private readonly Stopwatch ProcessStopWatch = new Stopwatch();
    public LogAndAnalyzeDynamicBidVariablesHandler(IChainHandler<AdMatchingModel> next, IRuleEngine ruleEngine, DebugLogger debugLogger)
    {
        var decoratedClass = new DynamicBidVariablesHandler(next, ruleEngine,debugLogger);
