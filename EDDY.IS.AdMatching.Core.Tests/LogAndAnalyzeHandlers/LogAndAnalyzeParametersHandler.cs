...
public class LogAndAnalyzeParametersHandler : IChainHandler<AdMatchingModel>, IStopWatchable
{
    public IChainHandler<AdMatchingModel> DecoratedClass { get; set; }
private readonly Stopwatch ProcessStopWatch = new Stopwatch();
    public LogAndAnalyzeParametersHandler(IChainHandler<AdMatchingModel> next, DebugLogger debugLogger)
    {
        var decoratedClass = new ParametersHandler(next, debugLogger);
