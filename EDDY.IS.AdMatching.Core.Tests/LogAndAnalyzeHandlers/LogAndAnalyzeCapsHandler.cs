...
public class LogAndAnalyzeCapsHandler : IChainHandler<AdMatchingModel>, IStopWatchable
{
    public IChainHandler<AdMatchingModel> DecoratedClass { get; set; }
private readonly Stopwatch ProcessStopWatch = new Stopwatch();
    public LogAndAnalyzeCapsHandler(IChainHandler<AdMatchingModel> next, DebugLogger debugLogger)
    {
        var decoratedClass = new CapsHandler(next, debugLogger);
