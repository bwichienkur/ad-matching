...
public class LogAndAnalyzeResponseBuilderHandler : IChainHandler<AdMatchingModel>, IStopWatchable
{
    public IChainHandler<AdMatchingModel> DecoratedClass { get; set; }
private readonly Stopwatch ProcessStopWatch = new Stopwatch();
    public LogAndAnalyzeResponseBuilderHandler(IChainHandler<AdMatchingModel> next, DebugLogger debugLogger)
    {
        var decoratedClass = new ResponseBuilderHandler(next, debugLogger);
