...
{
    public class StopsHandler : IChainHandler<AdMatchingModel>
    {
        private readonly DebugLogger _debugLogger;
        private readonly ILogger<StopsHandler> _logger;
        ...
        public async Task Handle(AdMatchingModel model)
        {
                ...
                StopsEvaluator stopsEval = new StopsEvaluator(_debugLogger);

                model.MainDictionaryEvaluated = stopsEval.Process(model.Filtered, model.MainDictionaryEvaluated, model.Parameters);

                _performanceLog.LogPerformanceDetail(model.SearchGuid, 1, "StopsHandler", startTime);
